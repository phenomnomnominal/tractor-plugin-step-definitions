...
describe('tractor-plugin-step-definitions: init:', () => {
    it('should create the features directory', () => {
        ...
        return init(config)
        .then(() => {
            expect(fs.mkdirAsync).to.have.been.calledWith('/tractor/step-definitions');
        })
        .finally(() => {
    ...
    it('should just move on if the directory already exists', () => {
        ...
        return init(config)
        .then(() => {
            expect(tractorLogger.warn).to.have.been.calledWith('"/tractor/step-definitions" directory already exists. Moving on...');
        })
        .finally(() => {
            fs.mkdirAsync.restore();
            process.cwd.restore();
